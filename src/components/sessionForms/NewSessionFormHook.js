...
function NewSessionFormHook (props) {
  ...
  const onSubmit = (data) => {
    ...
    fetch (`http://localhost:4000/api/v1/hobbies/${hobbyId}/sessions`, {
      ...
      .then(() => props.closeNewSessionModal())
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="px-4 max-w-3xl mx-auto">
      ...
      <div>
        ...
        <input className="border border-gray-400 block py-2 px-4 w-full rounded" type="text" name="sessionGoals" id="sessionGoals" ref={register} /> <br/>
      </div>
      <div>
        ...
        <div>
          ...
          <textarea className="border border-gray-400 block py-2 px-4 w-full rounded" name="notes" id="notes" cols="30" rows="10" ref={register}></textarea> <br/>
        </div>
        <div>
          <label htmlFor="duration">How Long did you practice for?(In Minutes)</label>
          <input className="border border-gray-400 block py-2 px-4 w-full rounded" type="number" name="duration" id="duration" ref={register} />
        </div>
          <div className="flex flex-row justify-around">
            <button type="submit" className="border py-2 px-4 bg-green-400 hover:bg-green-200" >Add Session</button>
            <button className="bg-red-400 ml-2 px-3 py-1" onClick={() => props.closeNewSessionModal()} >cancel</button>
          </div>
